...
void KillEnemy(struct enemy *t)
{
	...
	if ((ct - lastkill) > 100) {
		SND_Pos(DATADIR "/a/enemyhit.wav", 128, PlayerDist(t->x, t->y));
		lastkill = ct;
	}
...
void EnemySound(int t, int dist)
{
	...
	switch (t) {
		case 0:
			SND_Pos(DATADIR "/a/mons0shot.wav", 48, dist);
			last_delay = 200;
		break;
		case 1:
			SND_Pos(DATADIR "/a/mons1shot.wav", 112, dist);
			last_delay = 500;
		break;
		case 2:
			SND_Pos(DATADIR "/a/mons2shot.wav", 110, dist);
			last_delay = 1000;
		break;
		case 3:
			SND_Pos(DATADIR "/a/mons3shot.wav", 110, dist);
			last_delay = 500;
		break;
		case 4:
			SND_Pos(DATADIR "/a/mons4shot.wav", 110, dist);
			last_delay = 900;
		break;
		case 5:
			SND_Pos(DATADIR "/a/mons5shot.wav", 80, dist);
			last_delay = 60;
		break;
		case 6:
			SND_Pos(DATADIR "/a/mons6shot.wav", 110, dist);
			last_delay = 1000;
		break;
		case 7:
			SND_Pos(DATADIR "/a/mons7shot.wav", 110, dist);
			last_delay = 600;
		break;
		case 8:
			SND_Pos(DATADIR "/a/mons8shot.wav", 110, dist);
			last_delay = 700;
		break;
		case 9:
			SND_Pos(DATADIR "/a/mons9shot.wav", 110, dist);
			last_delay = 242;
		break;
		case 10:
			SND_Pos(DATADIR "/a/mons10shot.wav", 110, dist);
			last_delay = 250;
		break;
						...
						if (player_dying == 0) {
							if (PlayerDist(fx, fy) < 30) {
								...
								if (PlayerDist(fx, fy) < 4 - (2 * artifacts[5])) {
									player_dying = 1;
									SND_Pos(DATADIR "/a/playerhurt.wav", 128, 0);
									e->dying = 1;
									break;
								}
							}
						...
						if (shield_hp > 0) {
							...
							if (e->img == 4) {
								...
								if ((c_shield_hit_sound - 150) > last_shield_hit_sound) {
									SND_Pos(DATADIR "/a/shieldhit.wav", 50, 0);
				...
				if (PlayerDist(g->x, g->y) < 20) {
					...
					if (g->value == 31337) {
						if (player_hp < (3 + (player_shield == 30)*3)) {
							SND_Pos(DATADIR "/a/crystal.wav", 64, 0);
							player_hp++;
						} else {
							if (!training) {
								SND_Pos(DATADIR "/a/tone.wav", 64, 0);
