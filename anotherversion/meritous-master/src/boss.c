...
char *artifact_names[] = { 	N_("Holy Sword 'Balmung'"),
				N_("Mystic Halberd 'Amenonuhoko'"),
				N_("Divine Bow 'Gandiva'"),
				N_("Cursed Seal of Yahveh")};
...
//      "This string of text is precisely seventy eight characters in length, usefully."
{
  ...
  { // Wervyn Anixil
	...
	{1, 4, 0,
		N_("MERIT:\n"
		"  \n"
		"  The tainted PSI has driven even you mad. Mad with power. This is a blow\n"
...
void BossDialog()
{
	...
	if (boss_dlg > 0) {
		...
		draw_text(8, ypos+8, i18n_nonempty(dtext[dialog_set][boss_dlg-1].txt), 255);
	}
}
...
void TryHurtBoss(int x, int y, int range, int power)
{
	...
	if (dist(x, y, boss_x, boss_y) <= range) {
		...
		if (atk_power > boss_breakpoint) {
			...
			SND_Pos(DATADIR "/a/enemyhit.wav", 128, dist(x, y, boss_x, boss_y) / 4);
			boss_flash = 40;
			if (boss_hp <= 0) {
				boss_new_life = 1;
				boss_hp = 0;
