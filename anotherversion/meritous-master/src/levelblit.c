...
char save_filename[1024] = "SaveFile.sav";
...
int main(int argc, char **argv)
{
	...
	setlocale(LC_ALL, "");
	bindtextdomain("meritous", LOCALEDIR);
	textdomain("meritous");
...
/*			if (!strcasecmp(argv[i], "record")) {
	...
	if (envhome) {
		snprintf(save_filename, sizeof(save_filename), "%s/%s",
		         envhome, ".meritous.sav");
	}
#endif
	...
	pulse = malloc(SCREEN_W * SCREEN_H * sizeof(int));
	...
	while (executable_running) {
		...
		if (IsSaveFile(save_filename)) {
			/* See if we have the old or the new filename */
			can_continue = 1;
		} else {
			can_continue = 0;
		}
		...
		while (on_title) {
			...
			draw_text((SCREEN_W - 14*8)/2, 310 + can_continue*10, _("New Game"), 255);
			draw_text((SCREEN_W - 14*8)/2, 320 + can_continue*10, _("New Game (Wuss mode)"), 255);
...
void LoadingScreen(int part, float progress)
{
	...
	ProgressBarScreen(part, progress, _("Loading... please wait"), t_parts);
	ClearInput();
}

void SavingScreen(int part, float progress)
{
	ProgressBarScreen(part, progress, _("Saving... please wait"), 4.0);
	ClearInput();
}
...
int DungeonPlay(char *fname)
{
	...
	if (game_load) {
		first_game = 0;
		ReadPlayerData();
		//Paint(rooms[0].x+1, rooms[0].y+1, rooms[0].w-2, rooms[0].h-2, DATADIR "/d/fbossroom.loc");
	} else {
	...
	while (game_running) {
			...
			if (player_room == agate_knife_loc) {
				{
					...
					if (dist(player_x, player_y, xpos, ypos) < 20) {
						...
						SND_Pos(DATADIR "/a/crystal2.wav", 128, 0);
		...
		if (!tele_select) {
			sprintf(buf, _("Psi Crystals: %d"), player_gems);
			draw_text(3, 3, buf, 200);
			sprintf(buf, _("Explored: %.1f%% (%d/%d rooms)"), (float)explored/30.0, explored, 3000);
			draw_text(3, 11, buf, 200);
			sprintf(buf, _("Cleared: %.1f%% (%d/%d monsters)"), (float)killed_enemies/(float)total_enemies*100.0, killed_enemies, total_enemies);
			draw_text(3, 19, buf, 200);
			
			draw_text(316, 3, _("Reflect shield"), (player_gems >= UpgradePrice(0))&&(player_shield!=30) ? (231 + (t%13)*2) : 200);
			DrawMeter(434, 3, player_shield);
			
			draw_text(316, 11, _("Circuit charge"), (player_gems >= UpgradePrice(1))&&(circuit_fillrate!=30) ? (231 + (t%13)*2) : 200);
			DrawMeter(434, 11, circuit_fillrate);
			
			draw_text(316, 19, _("Circuit refill"), (player_gems >= UpgradePrice(2))&&(circuit_recoverrate!=30) ? (231 + (t%13)*2) : 200);
			DrawMeter(434, 19, circuit_recoverrate);
			
		} else {
#ifdef __ANDROID__
			draw_text(80, 11-6, _("Use the movement keys to locate a checkpoint. Press button 1 to"), 240);
			draw_text(52, 11+6, _("teleport to this checkpoint. Press Back or 3 once you are done."), 240);
#else
			draw_text(80, 11-6, _("Use the movement keys to locate a checkpoint. Press ENTER to"), 240);
			draw_text(52, 11+6, _("teleport to this checkpoint. Press ESCAPE or TAB once you are done."), 240);
#endif
		}
		...
		if (game_paused && (!map_enabled) && (!voluntary_exit)) {
			...
			draw_text((640 - 6 * 8) / 2, (480 - 8) / 2, _("Paused"), 255);
		...
		if (voluntary_exit) {
			...
			draw_text((640 - 30 * 8) / 2, (480 - 8) / 2 - 4, _("Are you sure you want to quit?"), 255);
#ifdef __ANDROID__
			draw_text((640 - 26 * 8) / 2, (480 - 8) / 2 + 4, _("Press button 2 to confirm."), 255);
#else
			draw_text((640 - 23 * 8) / 2, (480 - 8) / 2 + 4, _("Press enter to confirm."), 255);
#endif
		}
...
void ActivateBossDoor(int x, int y)
{
	...
	if (artifacts[8 + rooms[GetRoom(bx, by)].room_param]) {
		...
		if ((SDL_GetTicks() - bd_timer) > 100) {
			SND_Pos(DATADIR "/a/crystal2.wav", 100, 0);
...
void text_init()
{
	FILE *font_data_file;
	int chr, x, y;
	font_data_file = fopen(DATADIR "/d/font.dat", "rb");
...
void DrawCircuit()
{
	...
	draw_text(3, 469, _("Psi Circuit"), 200);
}
		...
		while (!given_treasure) {
			...
			switch (treasure) {
				case 0:
					...
					given_treasure = 1;
					SND_Pos(DATADIR "/a/tone.wav", 128, 0);
					break;
				case 1:
					if (player_shield < 25) {
						specialmessage = 10;
	...
	switch (tile) {
		...
		case 28:
			...
			if (player_gems >= UpgradePrice(0)) {
				...
				player_shield += 1;
				SND_Pos(DATADIR "/a/crystal.wav", 128, 0);
			}
			break;
		case 29:
		...
		case 32:
			CrystalSummon();
			SND_Pos(DATADIR "/a/crystal.wav", 80, 0);
			break;
		default:
			break;
	}
...
void SpecialTile(int x, int y)
{
	...
	switch (tile) {
		case 25:
			if (artifacts[11]) {
				sprintf(message, _("This is a checkpoint, but it doesn't seem to be working") );
				break;
			}
			if (checkpoints_found <= 1) {
				sprintf(message, _("This is a checkpoint. You will return here when you die."));
			} else {
#ifdef __ANDROID__
				sprintf(message, _("Press button 2 to teleport between checkpoints.") );
#else
				sprintf(message, _("Press ENTER to teleport between checkpoints.") );
#endif
			}
			break;
		case 26:
#ifdef __ANDROID__
			sprintf(message, _("Press button 2 to open the storage chest") );
#else
			sprintf(message, _("Press ENTER to open the storage chest") );
#endif
			break;
		case 28:
			if (player_shield >= 25) {
				sprintf(message, _("Your shield is already at full efficiency") );
			} else {
#ifdef __ANDROID__
				sprintf(message, _("Press button 2 to upgrade shields (%d crystals)"), UpgradePrice(0));
#else
				sprintf(message, _("Press ENTER to upgrade shields (%d crystals)"), UpgradePrice(0));
#endif
			}
			break;
		case 29:
			if (circuit_fillrate >= 25) {
				sprintf(message, _("Your circuit charge rate is already at its highest") );
			} else {
#ifdef __ANDROID__
				sprintf(message, _("Press button 2 to upgrade circuit charge (%d crystals)"), UpgradePrice(1));
#else
				sprintf(message, _("Press ENTER to upgrade circuit charge (%d crystals)"), UpgradePrice(1));
#endif
			}
			break;
		case 30:
			if (circuit_recoverrate >= 25) {
				sprintf(message, _("Your circuit refill rate is already at its highest") );
			} else {
#ifdef __ANDROID__
				sprintf(message, _("Press button 2 to upgrade circuit refill (%d crystals)"), UpgradePrice(2));
#else
				sprintf(message, _("Press ENTER to upgrade circuit refill (%d crystals)"), UpgradePrice(2));
#endif
			}
			break;
		case 31:
#ifdef __ANDROID__
			sprintf(message, _("Press button 2 to record your progress") );
#else
			sprintf(message, _("Press ENTER to record your progress") );
#endif
			break;
		case 32:
			if (total_gems == 0) {
				sprintf(message, _("This is a crystal device. It isn't working at the moment.") );
			} else {
#ifdef __ANDROID__
				sprintf(message, _("Press button 2 to activate the crystal device") );
#else
				sprintf(message, _("Press ENTER to activate the crystal device") );
#endif
			}
			break;
		case 42:
		...
		default:
			if (first_game) {
				if (otext < 60) {
#ifdef __ANDROID__
					sprintf(message, _("Press 4 to read the help file") );
#else
					sprintf(message, _("Press H to read the help file"));
#endif
					otext++;
				}
			}
	...
	if (message[0] == 0) {
		if (specialmessage != 0) {
			switch (specialmessage) {
				case 1: sprintf(message, _("Ancient artifact: Complete Map") ); break;
				case 2: sprintf(message, _("Ancient artifact: Shield boost") ); break;
				case 3: sprintf(message, _("Ancient artifact: Extra crystal efficiency") ); break;
				case 4: sprintf(message, _("Ancient artifact: Circuit booster") ); break;
				case 5: sprintf(message, _("Ancient artifact: Metabolism increase") ); break;
				case 6: sprintf(message, _("Ancient artifact: Dodge enhancer") ); break;
				case 7: sprintf(message, _("Ancient artifact: Ethereal Monocle") ); break;
				case 8: sprintf(message, _("Ancient artifact: Crystal gatherer") ); break;
				
				case 10: sprintf(message, _("Enhancement: Shield upgrade") ); break;
				case 11: sprintf(message, _("Enhancement: Circuit charge upgrade") ); break;
				case 12: sprintf(message, _("Enhancement: Circuit refill upgrade") ); break;
				
				case 20: sprintf(message, _("Reward: Psi crystals") ); break;
				
				case 30: sprintf(message, _("Holy Sword 'Balmung' answers your call") ); break;
				case 31: sprintf(message, _("Mystic Halberd 'Amenonuhoko' answers your call") ); break;
				case 32: sprintf(message, _("Divine Bow 'Gandiva' answers your call") ); break;
				case 33: sprintf(message, _("You capture the cursed seal. Return to the entrance") ); break;
				
				case 40: sprintf(message, _("Balmung will remain here, where the ley lines are strong") ); break;
				case 41: sprintf(message, _("Amenonuhoko will remain here, where the ley lines are strong") ); break;
				case 42: sprintf(message, _("Gandiva will remain here, where the ley lines are strong") ); break;
				
				case 50: sprintf(message, _(". . . . . .   retrieved 'Agate Knife'") ); break;
				
				default: sprintf(message, _("ERROR: NO MESSAGE VALUE GIVEN") ); break;
			}
			specialmessagetimer--;
